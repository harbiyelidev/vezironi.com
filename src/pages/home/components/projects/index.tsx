    ...
    return (
        <div className='rounded-lg border bg-[hsl(var(--bg-card))] text-[hsl(var(--text-foreground))] shadow-sm'>
            <div className='flex flex-col space-y-1.5 p-6'>
                ...
                <p className="text-sm text-[hsl(var(--text-muted-foreground))] font-['Geist-Regular']">You can find all my projects here.</p>
            </div>
            <div className='p-6 py-0'>
                <div className="relative grid max-h-80 grid-cols-1 gap-3 overflow-hidden md:grid-cols-2">
                    {config.projects.map(({ name, description, date, image, url }) => (
                        <div key={name} className="rounded-sm border bg-[hsl(var(--bg-card))] text-[hsl(var(--text-foreground))] shadow-sm relative h-full p-3 flex flex-col items-center justify-between">
